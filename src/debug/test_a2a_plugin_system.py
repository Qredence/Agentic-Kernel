...
class CalculatorAgent(BaseAgent):
    ...
    async def _handle_task_request(self, message: Message):
        ...
        logger.info(
            f"Agent {self.agent_id} received task request from {message.sender}"
        )
        ...
        if "calculate" in task_description.lower():
            ...
            calculator_plugin = await self.plugin_registry.get_plugin(
                "simple_calculator"
            )
...
class UserAgent(BaseAgent):
    ...
    async def _handle_task_response(self, message: Message):
        ...
        logger.info(
            f"Agent {self.agent_id} received task response from {message.sender}"
        )
    ...
    async def request_calculation(
        self, recipient: str, operation: str, a: float, b: float
    ):
