...
class ConflictManager:
    ...
    def propose_resolution(
        ...
        if conflict_id not in self.active_conflicts:
            logger.warning(
                f"Cannot propose resolution: conflict {conflict_id} not found"
            )
            return False
    ...
    def resolve_conflict(
        ...
        if not verification_result:
            logger.warning(
                f"Cannot resolve conflict: verification failed for {conflict_id}"
            )
            return False
    ...
    def suggest_resolution_strategy(
        ...
        if not conflict:
            return (
                ...
                "Default to consensus for unknown conflicts",
            )
        ...
        if conflict.severity == ConflictSeverity.CRITICAL:
            return (
                ...
                "Critical conflicts require authoritative resolution",
            )

        if conflict.conflict_type == ConflictType.RESOURCE_ALLOCATION:
            return (
                ResolutionStrategy.PRIORITIZATION,
                "Resource conflicts are best resolved through prioritization",
            )

        if len(conflict.parties) > 2:
            return (
                ResolutionStrategy.MEDIATION,
                "Multi-party conflicts benefit from mediation",
            )

        return (
            ResolutionStrategy.NEGOTIATION,
            "Direct negotiation is appropriate for simple conflicts",
        )
