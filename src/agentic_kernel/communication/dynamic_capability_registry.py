...
class CapabilitySubscription:
    ...
    def matches(self, capability: AgentCapability, agent_id: str) -> bool:
        ...
        if (
            ...
            and capability.capability_type not in self.capability_types
        ):
            return False
...
class AgentPresenceMonitor:
    ...
    def __init__(
        ...
        self.agent_statuses: dict[
            str, str
        ] = {}  # Maps agent IDs to their current status
...
class DynamicCapabilityRegistry(CapabilityRegistry):
    ...
    def unsubscribe(self, subscription_index: int) -> bool:
        ...
        if 0 <= subscription_index < len(self.subscriptions):
            ...
            logger.info(
                f"Agent {subscription.subscriber_id} unsubscribed from capabilities"
            )
            return True
        return False
        ...
        async with self._lock:
            ...
            for agent_id, agent_info in self.agents.items():
                ...
                for capability in agent_info.capabilities:
                    ...
                    if (
                        capability_types
                        and capability.capability_type not in capability_types
                    ):
                        continue
    ...
    async def broadcast_capability_update(
        ...
        if not agent_info:
            logger.warning(
                f"Cannot broadcast capability update: agent {agent_id} not registered"
            )
            return None
    ...
    async def _notify_subscribers(
        self, capability: AgentCapability, agent_id: str
    ) -> None:
            ...
            if subscription.matches(capability, agent_id):
                ...
                if subscription.callback:
                    try:
                        ...
                        logger.error(
                            f"Error in capability subscription callback: {str(e)}"
                        )
