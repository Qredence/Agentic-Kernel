...
class A2AResponseError(A2AClientError):
    ...
    def __init__(
        self, message: str, error_code: int | None = None, error_data: Any | None = None
    ):
...
class A2AClient:
    ...
    async def tasks_send_subscribe(
        ...
        async for event in self._stream_request(
            "tasks/sendSubscribe", params.dict(exclude_none=True)
        ):
            yield event
    ...
    async def tasks_cancel(self, task_id: str) -> Task:
        ...
        result = await self._make_request(
            "tasks/cancel", params.dict(exclude_none=True)
        )
        return Task(**result)

    async def tasks_push_notification_set(
        ...
        """
        result = await self._make_request(
            "tasks/pushNotification/set", config.dict(exclude_none=True)
        )
        return result
    ...
    async def tasks_push_notification_get(self, task_id: str) -> dict[str, Any]:
        ...
        params = TaskIdParams(id=task_id)
        result = await self._make_request(
            "tasks/pushNotification/get", params.dict(exclude_none=True)
        )
        return result
    ...
    async def tasks_resubscribe(
        ...
        params = TaskQueryParams(id=task_id, history_length=history_length)
        async for event in self._stream_request(
            "tasks/resubscribe", params.dict(exclude_none=True)
        ):
            yield event
