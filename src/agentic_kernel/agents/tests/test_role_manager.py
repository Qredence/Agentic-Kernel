...
class TestAgentRole(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            role.required_capabilities, {"data_processing", "file_handling"}
        )
    ...
    def test_to_dict(self):
        ...
        self.assertEqual(
            set(role_dict["required_capabilities"]),
            {"data_processing", "file_handling"},
        )
...
class TestRoleManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_assign_role_to_agent(self):
        ...
        result = await self.role_manager.assign_role_to_agent(
            "agent1", "data_processor"
        )
        ...
        result = await self.role_manager.assign_role_to_agent(
            "non_existent_agent", "data_processor"
        )
        self.assertFalse(result)
    ...
    async def test_unassign_role_from_agent(self):
        ...
        # Unassign role
        result = await self.role_manager.unassign_role_from_agent(
            "agent1", "data_processor"
        )
    ...
    async def test_find_agents_for_role(self):
        ...
        self.registry.get_all_agents = AsyncMock(
            return_value=[self.agent1, self.agent2, self.agent3]
        )
