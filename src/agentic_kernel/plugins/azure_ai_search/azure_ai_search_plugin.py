...
class AzureAISearchPlugin:
    ...
    def __init__(self) -> None:
        ...
        if not endpoint or not key or not self.index_name:
            logger.error(
                "Azure AI Search environment variables (ENDPOINT, KEY, INDEX_NAME) not fully set."
            )
        ...
        try:
            ...
            self.search_client = SearchClient(
                endpoint=endpoint, index_name=self.index_name, credential=credential
            )
    ...
    async def vector_search(
        ...
        top_k: int = Field(
            default=5, description="The number of top results to retrieve."
        ),
        ...
        select_fields: list[str] | None = Field(
            default=None, description="List of fields to retrieve from the documents."
        ),
        filter_expression: str | None = Field(
            default=None, description="OData filter expression to apply."
        ),
    ) -> str:
        ...
        try:
            vector_query = VectorizedQuery(
                vector=query_vector, k_nearest_neighbors=top_k, fields=vector_field_name
            )

            logger.info(
                f"Performing vector search on index '{self.index_name}' with top_k={top_k} on field '{vector_field_name}'. Filter: '{filter_expression}'"
            )
            ...
            async for result in search_results:
                doc = (
                    {field: result.get(field) for field in select_fields}
                    if select_fields
                    else dict(result)
                )
                # Include the score
