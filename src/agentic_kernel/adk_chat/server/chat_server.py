...
class ChatServer:
    ...
    def __init__(
        ...
        """
        logger.info("ChatServer.__init__: Initializing...")
        self.host = host
        self.port = port

        logger.info("ChatServer.__init__: Setting up environment...")
        # Set up environment
        setup_adk_a2a_environment()

        logger.info("ChatServer.__init__: Initializing managers...")
        # Initialize managers
        self.coordination_manager = CoordinationManager()
        self.trust_manager = TrustManager()

        logger.info("ChatServer.__init__: Initializing agents...")
        # Initialize agents
        self.agents = {}
        self.initialize_agents()

        logger.info("ChatServer.__init__: Setting up active connections...")
        # Initialize active connections
        self.active_connections: list[WebSocket] = []

        logger.info("ChatServer.__init__: Creating Starlette app...")
    ...
    async def home(self, _request: Request) -> JSONResponse:
    ...
    async def get_agents(self, _request: Request) -> JSONResponse:
    ...
    async def websocket_endpoint(self, websocket: WebSocket) -> None:
        ...
        try:
            while True:
                ...
                # Process message with orchestrator
                try:
                    response = await self.agents["orchestrator"].process_user_message(
                        message
                    )
