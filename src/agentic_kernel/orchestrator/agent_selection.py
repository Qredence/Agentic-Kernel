...
class AgentSkillMatrix:
    ...
    async def register_agent_capabilities(self, agent: BaseAgent) -> None:
        ...
        self.agent_capabilities[agent.agent_id] = capabilities.get(
            "supported_tasks", {}
        )
        logger.info(
            f"Registered capabilities for agent {agent.type} ({agent.agent_id})"
        )
    ...
    def update_agent_performance(
        ...
        if agent_id not in self.agent_performance:
            logger.warning(
                f"Attempting to update performance for unknown agent: {agent_id}"
            )
            return
        ...
        # Update success rate
        new_success_rate = (
            success_rate * (1 - alpha) + (1.0 if success else 0.0) * alpha
        )
    ...
    async def _select_agent_by_capability(
        ...
        for agent_id, agent in available_agents.items():
            ...
            for cap_name, cap_info in agent_caps.items():
                ...
                param_match_ratio = len(task_params.intersection(cap_params)) / max(
                    len(task_params), 1
                )
    ...
    def _select_best_performing_agent(self, agent_ids: List[str]) -> str:
        ...
        for agent_id in agent_ids:
            if agent_id in self.agent_performance:
                ...
                perf_score = (perf["success_rate"] * 0.8) + (
                    1.0 / (1.0 + perf["avg_execution_time"])
                ) * 0.2
    ...
    def _fallback_selection(
        ...
        for agent_id in available_agents:
            if agent_id in self.skill_matrix.agent_performance:
                success_rate = self.skill_matrix.agent_performance[agent_id][
                    "success_rate"
                ]
