...
class WorkflowHistory:
    ...
    async def get_version_history(self, workflow_id: str) -> List[Dict[str, Any]]:
        ...
        versions = sorted(self.versions[workflow_id], key=lambda v: v.created_at)
        ...
        return [
            {
                ...
                "is_current": v.version_id
                == self.workflows[workflow_id].get("current_version_id"),
            }
            for v in versions
        ]

    async def set_current_version(self, workflow_id: str, version_id: str) -> bool:
    ...
    async def record_step_result(
        ...
        logger.debug(
            f"Recorded result for step {step_name} in execution {execution_id}"
        )
    ...
    async def get_execution_history(
        ...
        executions = sorted(
            self.executions[workflow_id], key=lambda e: e.start_time, reverse=True
        )
