...
def mock_config():
    ...
    return MagicMock(
        ...
        web_config={"timeout": 30, "max_pages": 5},
        file_config={
...
def agent_system(mock_config):
    ...
    coder.execute_task = AsyncMock(
        return_value={"status": "success", "output": "Code generated successfully"}
    )
    terminal.execute_task = AsyncMock(
        return_value={"status": "success", "output": "Command executed successfully"}
    )
    file_surfer.execute_task = AsyncMock(
        return_value={"status": "success", "output": "File operations completed"}
    )
    web_surfer.execute_task = AsyncMock(
        return_value={"status": "success", "output": "Web content retrieved"}
    )
...
def orchestrator(agent_system, mock_config):
    ...
    orchestrator = OrchestratorAgent(
        config=mock_config, task_ledger=task_ledger, progress_ledger=progress_ledger
    )
...
async def test_coder_terminal_integration(agent_system, orchestrator):
    ...
    terminal = next(
        a for a in agent_system.get_all_agents() if isinstance(a, TerminalAgent)
    )
...
async def test_web_surfer_coder_integration(agent_system, orchestrator):
    ...
    # Get the agents from the system
    web_surfer = next(
        a for a in agent_system.get_all_agents() if isinstance(a, WebSurferAgent)
    )
    coder = next(a for a in agent_system.get_all_agents() if isinstance(a, CoderAgent))
    ...
    workflow = [
        ...
        WorkflowStep(
            task=Task(
                ...
                parameters={
                    "language": "python",
                    "api_docs": "Will be filled from previous step",
                },
            ),
...
async def test_file_surfer_terminal_integration(agent_system, orchestrator):
    ...
    # Get the agents from the system
    file_surfer = next(
        a for a in agent_system.get_all_agents() if isinstance(a, FileSurferAgent)
    )
    terminal = next(
        a for a in agent_system.get_all_agents() if isinstance(a, TerminalAgent)
    )
...
async def test_full_development_workflow_integration(agent_system, orchestrator):
    ...
    web_surfer.execute_task = AsyncMock(
        side_effect=lambda task: {
            ...
            "content": {
                "api_docs": [{"name": "Example API", "endpoint": "/api/v1/data"}]
            },
        }
    )
        ...
        WorkflowStep(
            task=Task(
                ...
                parameters={
                    ...
                    "spec": "Will be filled from design",
                },
            ),
    ...
    terminal_calls = [
        call.args[0].name for call in terminal.execute_task.call_args_list
    ]
    ...
    assert terminal_calls.index("run_tests") < terminal_calls.index(
        "deploy_application"
    )
...
async def test_error_recovery_across_agents(agent_system, orchestrator):
    ...
    coder_results = [
        {
            "status": "success",
            "output": "Initial code generated",
            "files": [{"name": "script.py", "content": "# Buggy code"}],
        },
        {
            "status": "success",
            "output": "Fixed code generated",
            "files": [{"name": "script.py", "content": "# Fixed code"}],
        },
    ]
    ...
    async def mock_execute_step(step, context=None):
        ...
        if step.task.name == "run_code" and result.get("status") == "error":
            ...
            if step.task.error_handler:
                error_step = next(
                    (s for s in workflow if s.task.name == step.task.error_handler),
                    None,
                )
                if error_step:
