...
from agentic_kernel.plugins.file_surfer import (
    FileInfo,
)  # Needed for mocking return type
...
def mock_plugin():
    ...
    plugin.search_files.return_value = [
        MOCK_FILE_INFO_LIST[0]
    ]  # Assume only first file matches search
    return plugin
...
async def test_file_surfer_agent_initialization(MockFileSurferPlugin, mock_plugin):
    ...
    agent = FileSurferAgent(
        config={"plugin_config": {"base_path": str(MOCK_BASE_PATH)}}
    )
...
async def test_execute_list_task(MockFileSurferPlugin, mock_plugin):
    ...
    mock_plugin.list_files.assert_called_once_with(
        pattern=MOCK_LIST_PATTERN, recursive=False
    )
...
async def test_execute_search_task(MockFileSurferPlugin, mock_plugin):
    ...
    task_desc = (
        f"search for '{MOCK_SEARCH_TEXT}' in files matching {MOCK_SEARCH_PATTERN}"
    )
    ...
    expected_output = [
        MOCK_FILE_INFO_LIST[0].model_dump(mode="json")
    ]  # Only the first file matched
    assert result["output"]["files_found"] == expected_output
    mock_plugin.search_files.assert_called_once_with(
        text=MOCK_SEARCH_TEXT, file_pattern=MOCK_SEARCH_PATTERN
    )
    mock_plugin.list_files.assert_not_called()
    mock_plugin.read_file.assert_not_called()
