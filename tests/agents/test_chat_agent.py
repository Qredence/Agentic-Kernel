...
def mock_config_loader():
    ...
    loader.get_model_config.return_value = {"temperature": 0.7, "max_tokens": 1000}
    return loader
...
def chat_agent(mock_kernel, mock_config_loader):
    ...
    config = AgentConfig(
        name="test_chat_agent", model="gpt-4", endpoint="https://test.openai.azure.com/"
    )
    return ChatAgent(
...
async def test_chat_history_management(chat_agent, mock_kernel, mock_ai_service):
    ...
    assert (
        len(chat_agent.chat_history.messages) == 2
