...
from agentic_kernel.plugins.web_surfer import (
    WebSearchResult,
...
)  # Patch the plugin import within the agent file
...
async def test_execute_search_task(MockWebSurferPlugin, mock_plugin):
    ...
    assert result["output"]["search_results"] == [
        r.model_dump() for r in MOCK_SEARCH_RESULTS
    ]
...
@pytest.mark.asyncio
@patch("agentic_kernel.agents.web_surfer_agent.WebSurferPlugin")
async def test_execute_summarize_task_url_in_description(
    MockWebSurferPlugin, mock_plugin
):
...
async def test_handle_summarization_error(MockWebSurferPlugin, mock_plugin):
    ...
    assert result["status"] == "failure"
    assert (
        "An unexpected error occurred during summarization" in result["error_message"]
    )
...
@pytest.mark.asyncio
@patch("agentic_kernel.agents.web_surfer_agent.WebSurferPlugin")
async def test_handle_plugin_summarization_error_string(
    MockWebSurferPlugin, mock_plugin
):
