...
def mock_llm():
    ...
    mock.evaluate_progress = AsyncMock(
        return_value={"success_rate": 0.9, "needs_replanning": False, "suggestions": []}
    )
    # Make replan_task awaitable
    mock.replan_task = AsyncMock(
        return_value=[
            PlanStep(description="Alternative step", status="pending", depends_on=[])
        ]
    )
    return mock
...
def mock_agents():
    ...
    for name in ["WebSurferAgent", "FileSurferAgent", "CoderAgent", "TerminalAgent"]:
        ...
        mock_agent.execute_task = AsyncMock(
            return_value={"status": "success", "output": "Task completed"}
        )
...
@pytest.fixture
def progress_ledger():
    return ProgressLedger(task_id="test_task", current_status="not_started")


async def test_outer_loop_execution(
    orchestrator_agent, mock_llm, task_ledger, progress_ledger
):
...
async def test_outer_loop_replanning(
    orchestrator_agent, mock_llm, task_ledger, progress_ledger
):
    ...
    mock_llm.evaluate_progress.side_effect = [
        ...
        {"success_rate": 0.9, "needs_replanning": False, "suggestions": []},
    ]
...
async def test_outer_loop_task_retry(
    orchestrator_agent, mock_llm, task_ledger, progress_ledger
):
...
async def test_outer_loop_parallel_execution(
    orchestrator_agent, mock_llm, task_ledger, progress_ledger
):
    ...
    steps = [
        ...
        PlanStep(
            step_id="2", description="Analyze codebase", status="pending", depends_on=[]
        ),
        PlanStep(
            step_id="3",
            description="Generate implementation",
    ...
    result = await orchestrator_agent.execute_workflow(
        task_ledger, progress_ledger, allow_parallel=True
    )
...
async def test_outer_loop_error_handling(
    orchestrator_agent, mock_llm, task_ledger, progress_ledger
):
...
@pytest.mark.asyncio
async def test_outer_loop_progress_tracking(
    orchestrator_agent, mock_llm, mock_agents, task_ledger, progress_ledger
):
    ...
    mock_metrics = {"cpu_usage": 50.0, "memory_usage": 100.0, "duration": 1.0}
