...
import asyncio
import asyncpg
from src.agentic_kernel.memory.types import (
    MemoryEntry,
    MemoryType,
    MemoryStats,
    MemorySearchResult,
...
def embedding_config():
    ...
    return EmbeddingConfig(
        endpoint="http://test-endpoint", api_key="test-key", cache_embeddings=True
    )
...
async def memory_store(
    embedding_service, db_pool, migration_manager
) -> AsyncGenerator[PostgresMemoryStore, None]:
...
async def test_memory_persistence_search(memory_store):
    ...
    # Search by content
    results = await memory_store.search(query="Persistent memory 0", agent_id="agent1")
    assert len(results) > 0
    assert "Persistent memory 0" in results[0].entry.content

    # Search by type
    results = await memory_store.search(
        agent_id="agent1", memory_type=MemoryType.LONG_TERM
    )
    assert len(results) == 5
    assert all(r.entry.memory_type == MemoryType.LONG_TERM for r in results)

    # Search by tags
    results = await memory_store.search(agent_id="agent1", tags=["tag1"])
...
async def test_memory_persistence_sharing(memory_store):
    ...
    results = await memory_store.search(agent_id="agent2", include_shared=True)
...
async def test_concurrent_operations(memory_store):
    ...
    async def update_memory(memory_id: str, idx: int):
        return await memory_store.update(
            memory_id, {"content": f"Updated memory {idx}", "importance": 0.6}
        )
...
async def test_complex_search_scenarios(memory_store):
    ...
    memories = [
        MemoryEntry(
            **base_memory,
            content="Technical discussion about Python",
        ...
        MemoryEntry(
            **base_memory,
            content="Python code review feedback",
            tags=["technical", "python", "review"],
        ),
        MemoryEntry(
            **base_memory, content="Team lunch discussion", tags=["team", "social"]
        ),
    ]
    ...
    results = await memory_store.search(
        agent_id="agent1", tags=["technical", "python"], min_relevance=0.5
    )
    assert len(results) == 2

    # Test content and tag combination
    results = await memory_store.search(
        query="Python", agent_id="agent1", tags=["technical"], min_relevance=0.5
    )
    assert len(results) == 2

    # Test exclusion patterns
    results = await memory_store.search(
        agent_id="agent1", tags=["team"], query="technical", min_relevance=0.5
    )
    assert len(results) == 0
...
async def test_performance_under_load(memory_store):
    ...
    batch_size = 20
    tasks = [create_batch(i * batch_size, batch_size) for i in range(5)]
    await asyncio.gather(*tasks)

    # Perform concurrent searches
    async def search_memories(query: str):
        return await memory_store.search(query=query, agent_id="agent1", max_results=10)

    search_tasks = [search_memories(f"memory {i}") for i in range(10)]
