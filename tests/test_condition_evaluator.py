...
def test_conditional_branch_manager_step_execution():
    ...
    assert manager.should_execute_step("step3", "eq(${step_status.step1}, 'success')")
...
def test_ready_steps_with_conditions():
    ...
    ready_steps = manager.get_ready_steps_with_conditions(
        "test_workflow", workflow_steps, completed_steps
    )
...
def test_template_var_parsing():
    ...
    assert evaluator.evaluate(
        "contains('The value is ${nested.level1.level2.value}', 'test')"
    )
