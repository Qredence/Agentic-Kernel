...
from agentic_kernel.config_types import (
    AgentConfig,
    LLMMapping,
)  # Simple AgentConfig for single agents
...
async def agent_config():
    ...
    return AgentConfig(
        ...
        llm_mapping=LLMMapping(model="gpt-4", temperature=0.7, max_tokens=2000),
    )
...
async def mock_agents(agent_config):
    ...
    coder.execute = AsyncMock(
        return_value={"status": "success", "result": "Code generated"}
    )
    terminal.execute = AsyncMock(
        return_value={"status": "success", "result": "Command executed"}
    )
    file_surfer.execute = AsyncMock(
        return_value={"status": "success", "result": "File processed"}
    )
    web_surfer.execute = AsyncMock(
        return_value={"status": "success", "result": "Web content fetched"}
    )
...
async def orchestrator(agent_config, mock_agents):
    ...
    orchestrator = OrchestratorAgent(
        config=agent_config, task_ledger=task_ledger, progress_ledger=progress_ledger
    )
...
async def test_error_handling_and_recovery(orchestrator, mock_agents):
    ...
    mock_agents["terminal"].execute = AsyncMock(
        side_effect=[
            ...
            {"status": "success", "result": "Retry succeeded"},
        ]
    )
