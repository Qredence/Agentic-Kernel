...
def sample_team_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "security_policy": {
            ...
            "allowed_commands": ["ls", "cat"],
            "terminal_sandbox": {"image": "python:3.9-alpine", "network": "none"},
        },
    }
...
def sample_config_json(sample_team_config) -> str:
    ...
    config_data = {
        ...
        "default_model": "gpt-4-turbo-preview",
        "agent_teams": {"test_team": sample_team_config},
...
def test_programmatic_team_config():
    ...
    team_config = AgentTeamConfig(
        ...
        agents=[
            AgentConfig(
                ...
                config={"sandbox_type": "docker", "allowed_commands": ["ls", "cat"]},
            )
        ],
        security_policy=SecurityPolicy(
            allowed_commands=["ls", "cat"],
            terminal_sandbox=DockerSandboxConfig(image="alpine:latest", network="none"),
        ),
    )
    ...
    new_team = AgentTeamConfig(
        ...
        agents=[
            AgentConfig(
                name="web_surfer", type="WebSurferAgent", description="Web agent"
            )
        ],
    )
