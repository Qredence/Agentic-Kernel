...
async def test_memory_store_search(memory_store):
    ...
    # Search by content
    results = await memory_store.search(query="Memory 0", agent_id="agent1")
    assert len(results) > 0
    assert results[0].entry.content == "Memory 0"

    # Search by type
    results = await memory_store.search(
        agent_id="agent1", memory_type=MemoryType.SHORT_TERM
    )
...
async def test_memory_sharing(memory_store):
    ...
    results = await memory_store.search(agent_id="agent2", include_shared=False)
...
async def test_memory_manager_operations(memory_manager):
    ...
    results = await memory_manager.recall(agent_id="agent1", query="Test memory")
    assert len(results) > 0
    assert results[0].entry.content == "Test memory"

    # Update the memory
    updated = await memory_manager.update_memory(
        agent_id="agent1", memory_id=memory_id, updates={"importance": 0.8}
    )
    assert updated is not None
    assert updated.importance == 0.8
    ...
    shared = await memory_manager.share_memories(
        source_agent_id="agent1", target_agent_ids=["agent3"], memory_ids=[memory_id]
    )
    assert shared is True
...
async def test_embedding_service(embedding_service):
    ...
    assert all(
        len(embedding) == 1536 for embedding in embeddings
