...
async def orchestrator_with_agents(create_mock_agents):
    ...
    def _create_orchestrator(agent_configs, goal="Test complex workflow"):
        ...
        orchestrator = OrchestratorAgent(
            config=MagicMock(), task_ledger=task_ledger, progress_ledger=progress_ledger
        )
...
async def test_complex_workflow_with_multiple_dependencies(orchestrator_with_agents):
    ...
    executed_tasks = [
        call.args[0].name for call in agents["coder"].execute_task.call_args_list
    ]
    assert "design_architecture" in executed_tasks
    assert "implement_integration" in executed_tasks
    assert executed_tasks.index("design_architecture") < executed_tasks.index(
...
async def test_inter_agent_communication(orchestrator_with_agents):
    ...
    with patch.object(
        CommunicationProtocol, "send_message", new_callable=AsyncMock
    ) as mock_send:
        # Execute the workflow
        result = await orchestrator.execute_workflow(workflow)
        ...
        completion_messages = [
            msg
            for msg in coder_agent.messages_sent
...
async def test_conditional_workflow_execution(orchestrator_with_agents):
    ...
    # Create agents
    agent_configs = {"file_surfer": {"success": True}, "coder": {"success": True}}
    ...
    async def custom_execute(task):
        ...
        return {"status": "success", "output": f"Completed task: {task.name}"}
