...
async def test_task_list_sync_with_tasks():
    ...
    task_statuses = {
        task.title.split(":")[0].strip(): task.status for task in task_list.tasks
    }
    assert any(
        "Search Web" in title and status == MockTaskStatus.DONE
        for title, status in task_statuses.items()
    )
    assert any(
        "Read File" in title and status == MockTaskStatus.RUNNING
        for title, status in task_statuses.items()
    )
