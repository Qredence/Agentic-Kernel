...
async def test_update_task_status(task_manager):
    ...
    task = await task_manager.create_task(
        name="test_task", agent_type="test", description="Test task description"
    )

    await task_manager.update_task_status(task.id, "completed", {"result": "success"})
...
async def test_sync_with_chainlit_tasklist(task_manager, mock_tasklist):
    ...
    task1 = await task_manager.create_task(
        name="pending_task", agent_type="test", description="A pending task"
    )

    task2 = await task_manager.create_task(
        name="completed_task", agent_type="test", description="A completed task"
    )
    await task_manager.complete_task(task2.id, {"result": "success"})

    task3 = await task_manager.create_task(
        name="failed_task", agent_type="test", description="A failed task"
    )
    ...
    status_mapping = {task.title: task.status for task in mock_tasklist.tasks}
    assert any(
        "pending_task" in title and status == "running"
        for title, status in status_mapping.items()
    )
    assert any(
        "completed_task" in title and status == "done"
        for title, status in status_mapping.items()
    )
    assert any(
        "failed_task" in title and status == "failed"
        for title, status in status_mapping.items()
    )
