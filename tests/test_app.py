...
def mock_config_loader():
    ...
    mock.get_model_config.return_value = {
        ...
        "llm_mapping": {"model": "gpt-4o-mini", "endpoint": "azure_openai"},
        "temperature": 0.7,
        "max_tokens": 1000,
    }
    return mock
...
class TestChatAgent:
    ...
    def test_init(self, mock_kernel, mock_config_loader):
        ...
        config = AgentConfig(
            ...
            llm_mapping=LLMMapping(model="gpt-4o", endpoint="azure_openai"),
        )
        agent = ChatAgent(
            config=config, kernel=mock_kernel, config_loader=mock_config_loader,
        )
...
async def test_on_message_no_agent(mock_chainlit, mock_chainlit_context, mock_message):
    ...
    response_message.content = (
        "Chat agent not initialized properly. Please restart the chat."
    )
...
async def test_get_chat_profile():
    ...
    assert max_config["model"] == "gpt-4o"
    ...
    async def execute(self, task: Task) -> dict[str, TypingAny]:
        ...
        try:
            ...
            async for chunk in self.handle_message(task.parameters.get("message", "")):
                response.append(chunk)
            return {"status": "success", "result": "".join(response)}
        except Exception as e:
            return {"status": "error", "error": str(e)}

    async def handle_message(self, message: str) -> AsyncGenerator[str, TypingAny]:
        ...
        response = ""
        stream = self.kernel.get_service(
            "azure_openai",
