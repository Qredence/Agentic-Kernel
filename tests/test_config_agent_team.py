...
def valid_agent_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "llm_mapping": {"model": "gpt-4", "temperature": 0.7, "max_tokens": 1000},
        "config": {"max_search_results": 5, "timeout": 30},
    }
...
def test_invalid_agent_config():
    ...
    with pytest.raises(ValidationError):
        AgentConfig(name="test", type="UnknownAgent")
...
def test_llm_mapping_validation():
    ...
    # Valid mapping
    valid_mapping = LLMMapping(model="gpt-4", temperature=0.7, max_tokens=1000)
