...
from src.agentic_kernel.orchestrator.agent_selection import (
    AgentSelector,
    AgentSkillMatrix,
)
...
async def test_agent_selection_with_specialization(agent_selector, mock_agents):
    ...
    code_agent_id = next(
        agent_id for agent_id, agent in mock_agents.items() if agent.type == "code"
    )
...
async def test_orchestrator_agent_selection_integration():
    ...
    orchestrator.register_agent_specialization(
        web_agent.agent_id, ["search", "browsing"]
    )
    orchestrator.register_agent_specialization(
        code_agent.agent_id, ["development", "programming"]
    )
