...
def generate_workflow_report(
    ...
    if agent_stats:
        ...
        for agent, stats in agent_stats.items():
            ...
            report.append(
                f"| {agent} | {tasks} | {success_rate:.1f}% | {avg_duration:.2f} |"
            )
        report.append("")
...
def save_workflow_result(
    ...
    for task_name, task_data in result_copy.get("task_results", {}).items():
        if (
            "output" in task_data
            and isinstance(task_data["output"], str)
            and len(task_data["output"]) > 1000
        ):
...
if __name__ == "__main__":
    ...
    sample_result = {
        ...
        "failed_steps": ["task3"],
        "retried_steps": ["task3"],
        "metrics": {"success_rate": 0.75, "execution_time": 15.5},
        "task_results": {
            "task1": {
