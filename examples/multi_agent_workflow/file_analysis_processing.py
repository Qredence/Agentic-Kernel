...
async def main():
    ...
    if "generate_upgrade_plan" in result.get("task_results", {}):
        plan_content = result["task_results"]["generate_upgrade_plan"].get(
            "content", ""
        )
    ...
    if "generate_code_fixes" in result.get("task_results", {}):
        patch_files = result["task_results"]["generate_code_fixes"].get(
            "patch_files", []
        )
    ...
    if "analyze_project_structure" in result.get("task_results", {}):
        project_stats = result["task_results"]["analyze_project_structure"].get(
            "stats", {}
        )
        print("\nProject Statistics:")
    ...
    if "scan_dependencies" in result.get("task_results", {}):
        dependencies = result["task_results"]["scan_dependencies"].get(
            "dependencies", {}
        )
        print("\nDependencies:")
