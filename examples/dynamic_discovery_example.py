...
class SimpleCommunicationProtocol(CommunicationProtocol):
    ...
    async def send_message(self, message: Message) -> str:
        ...
        """
        logger.info(
            f"Sending message: {message.message_type.value} from {message.sender} to {message.recipient}"
        )
...
class SimpleAgent:
    ...
    async def register(self):
        ...
        logger.info(
            f"Agent {self.agent_id} registered with {len(self.capabilities)} capabilities"
        )
    ...
    async def discover_capabilities(self, capability_types=None):
        ...
        logger.info(
            f"Agent {self.agent_id} discovered {len(capabilities)} capabilities"
        )

        for cap in capabilities:
            logger.info(
                f"  - {cap['name']} ({cap['capability_type']}) from {cap['agent_id']}"
            )
    ...
    async def subscribe_to_capabilities(self, capability_types=None):
        ...
        logger.info(
            f"Agent {self.agent_id} subscribed to capability updates (index: {subscription_index})"
        )
    ...
    def capability_update_callback(self, capability, agent_id):
        ...
        """
        logger.info(
            f"Agent {self.agent_id} received capability update: {capability.name} from {agent_id}"
        )
...
async def main():
    ...
    try:
        # Create some agents
        agents = [
            SimpleAgent(f"agent-{i}", f"type-{i % 3}", registry) for i in range(5)
        ]
